...
<body>
	<h1>Extended Forecast for Freehold NJ</h1>
	...
	<?php
      ...
      $servername = "localhost";
		...
		foreach ($result as $row) {
			if ($row['set_homepage'] == 'freehold.php'){
				echo "<h2>";
    ...
    <form method="post" action="freehold.php">
...
<?php
	if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['cities']))
    {
		...
		$ip_insert = "INSERT INTO Personalization (ip, set_homepage) VALUES ('$ip', 'freehold.php')";
		$ip_update = "UPDATE Personalization SET set_homepage='freehold.php' WHERE ip='$ip'";
...
?>	
	...
	$result = $mysqli->query("SELECT periods, temps, short_desc, long_desc FROM Freehold");
	foreach ($result as $row) {
		$long = $row['long_desc'];
