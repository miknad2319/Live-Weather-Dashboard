...
<body>
	<h1>Extended Forecast for Wildwood NJ</h1>
	...
	<?php
      ...
      $servername = "localhost";
		...
		foreach ($result as $row) {
			if ($row['set_homepage'] == 'wildwood.php'){
				echo "<h2>";
   ...
   <form method="post" action="wildwood.php">
...
<?php
	if($_SERVER['REQUEST_METHOD'] == "POST" and isset($_POST['cities']))
    {
		...
		$ip_insert = "INSERT INTO Personalization (ip, set_homepage) VALUES ('$ip', 'wildwood.php')";
		$ip_update = "UPDATE Personalization SET set_homepage='wildwood.php' WHERE ip='$ip'";

		if ($mysqli->query($ip_insert) === TRUE){
			echo "New favorite recorded";
		}
...
?>	
   ...
   <br>
	...
	$result = $mysqli->query("SELECT periods, temps, short_desc, long_desc FROM Wildwood");
	foreach ($result as $row) {
		$long = $row['long_desc'];
